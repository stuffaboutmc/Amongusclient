...
public class KillAura extends Module {
    ...
    public void onUpdate() {
        ...
        if (!attackWhileMoving && (mc.thePlayer.moveForward != 0 || mc.thePlayer.moveStrafing != 0)) return;
        ...
        List<Entity> entities = mc.theWorld.loadedEntityList.stream()
                ...
                .filter(e -> {
                    if (e instanceof EntityPlayer) {
                        ...
                        if (!teams && mc.thePlayer.isOnSameTeam(p)) return false;
                        if (!invisibles && p.isInvisible()) return false;
                    } else {
                        if (!animals) return false;
                    }
                    if (mc.thePlayer.getDistanceToEntity(e) > range) return false;
                    if (!throughWalls && !mc.thePlayer.canEntityBeSeen(e)) return false;
        ...
        if (!entities.isEmpty()) {
            ...
            if (now - lastAttack > delay) {
                if (Math.random() * 100 < hitChance) {
                    if (autoCrit && mc.thePlayer.onGround && Math.random() > 0.3) {
                        InputSimulator.holdKey(KeyEvent.VK_SPACE, 50);
                    }
                    InputSimulator.clickLeft();
                    if (autoBlock) {
                        InputSimulator.holdKey(KeyEvent.VK_CONTROL, 30);
                    }
                    lastAttack = now;
                }
            }
