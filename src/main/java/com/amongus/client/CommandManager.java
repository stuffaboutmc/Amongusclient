...
public class CommandManager {
    ...
    public void onChat(ClientChatReceivedEvent event) {
        ...
        switch (cmd) {
            ...
            case "toggle": case "t": handleToggle(args); break;
            case "changeprefix": handleChangePrefix(args); break;
            case "help": handleHelp(); break;
            default: send("Unknown command."); break;
        }
    }
    ...
    private void handleBind(String[] args) {
        ...
        if (target == null) { send("Module not found."); return; }
        int keyCode = Keyboard.getKeyIndex(keyName);
        if (keyCode == Keyboard.KEY_NONE && !keyName.equals("NONE")) { send("Invalid key."); return; }
    ...
    private void handleChangePrefix(String[] args) {
        ...
        if (newPrefix.length() > 3) { send("Prefix too long."); return; }
        prefix = newPrefix;
        send("Prefix changed to " + prefix);
    }

    private void handleHelp() { send("Commands: bind, toggle, t, changeprefix, help"); }
    private void send(String message) { mc.thePlayer.addChatMessage(new ChatComponentText("§c[Amongus] §f" + message)); }
}
