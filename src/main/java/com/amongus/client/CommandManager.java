...
public class CommandManager {
    ...
    public static void setModules(List<Module> mods) {
        modules = mods;
    }
    ...
    private void handleBind(String[] args) {
        if (args.length < 3) {
            send("Usage: " + prefix + "bind <module> <key>");
            return;
        }
    ...
    private void handleToggle(String[] args) {
        if (args.length < 2) {
            send("Usage: " + prefix + "toggle <module>");
            return;
        }
        String moduleName = args[1].toLowerCase();
        for (Module m : modules) {
    ...
    private void handleChangePrefix(String[] args) {
        if (args.length < 2) {
            send("Usage: " + prefix + "changeprefix <new prefix>");
            return;
        }
        String newPrefix = args[1];
        if (newPrefix.length() > 3) {
            send("Prefix too long, 3 characters max");
            return;
        }
        prefix = newPrefix;
